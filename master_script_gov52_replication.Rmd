...
FL_post_pred <- predict(reg1, newdata = FL_post)
...
# making predictions for PA pre and post stafford act
...
PA_pre_pred <- predict(reg1, newdata = PA_pre)

PA_pre_1 <- PA_pre %>%
  cbind(PA_pre_pred) %>%
  mutate(yhat = PA_pre_pred) %>%
  select(-PA_pre_pred)

# post stafford act

PA_post_pred <- predict(reg1, newdata = PA_post)

PA_post_1 <- PA_post %>%
  cbind(PA_post_pred) %>%
  mutate(yhat = PA_post_pred) %>%
  select(-PA_post_pred)

# combining the prediction dfs

PA_comb <- rbind(PA_pre_1, PA_post_1)
...
PA_comb %>%
  ggplot(aes(x = comp, y = yhat)) +
  geom_point() +
  labs(title = "PA Competitiveness over Time") +
  theme_classic()
```
